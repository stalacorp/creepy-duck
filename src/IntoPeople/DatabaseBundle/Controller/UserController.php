                        ...
                        if (!$user) {
                            ...
                            if ($language != null) {
                                ...
                                $message = \Swift_Message::newInstance()
                                    ...
                                    ->setTo($user->getEmail())
                                    ->setBody(str_replace(array('$url', '$username', '$password'), array('http://' . $request->getHttpHost() . $this->generateUrl('user_firstlogin', array('token' => $password, 'id' => $user->getId())), $user->getEmail(), $password), $systemmail->getBody()), 'text/html');
            ...
            if ($userexists == null) {
                ...
                if ($systemmail->getMailtype()->getIsActive()) {
                    $message = \Swift_Message::newInstance()
                        ...
                        ->setBody(str_replace(array('$url', '$username', '$password'), array('http://' . $request->getHttpHost() . $this->generateUrl('user_firstlogin', array('token' => $password, 'id' => $entity->getId())), $entity->getEmail(), $password), $systemmail->getBody()), 'text/html');
